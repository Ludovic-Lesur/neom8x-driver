...
static uint8_t _NEOM8X_check_position(NEOM8X_position_t* gps_position) {
	...
	if ((gps_position -> lat_degrees  < 90)     &&
		(gps_position -> lat_minutes  < 60)     &&
		(gps_position -> lat_seconds  < 100000) &&
		(gps_position -> long_degrees < 180)    &&
		(gps_position -> long_minutes < 60)     &&
		(gps_position -> long_seconds < 100000))
	{
...
/*******************************************************************/
static void _NEOM8X_compute_nmea_checksum(char_t* nmea_rx_buf, uint8_t* ck) {
	// Local variables.
...
/*******************************************************************/
static NEOM8X_status_t _NEOM8X_get_nmea_checksum(char_t* nmea_rx_buf, uint8_t* ck) {
	// Local variables.
...
static NEOM8X_status_t _NEOM8X_parse_nmea_zda(char_t* nmea_rx_buf, NEOM8X_time_t* gps_time, uint8_t* decode_success_flag) {
	...
	// Compute checksums.
	status = _NEOM8X_get_nmea_checksum(nmea_rx_buf, &received_checksum);
	if (status != NEOM8X_SUCCESS) goto errors;
	_NEOM8X_compute_nmea_checksum(nmea_rx_buf, &computed_checksum);
	// Verify checksum.
	if (computed_checksum != received_checksum) goto errors;
...
NEOM8X_status_t NEOM8X_init(void) {
	...
	for (buffer_idx=0 ; buffer_idx<NEOM8X_NMEA_RX_BUFFER_DEPTH ; buffer_idx++) {
		for (idx=0 ; idx<NEOM8X_NMEA_RX_BUFFER_SIZE ; idx++) neom8x_ctx.nmea_buffer[buffer_idx][idx] = 0;
	}
...
NEOM8X_status_t NEOM8X_set_timepulse(NEOM8X_timepulse_configuration_t* timepulse_config){
	...
	uint8_t neom8x_cfg_tp5[NEOM8X_MSG_OVERHEAD_SIZE + NEOM8X_CFG_TP5_PAYLOAD_SIZE] = {
